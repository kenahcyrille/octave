...
def download_and_verify(url, pkg_download_dir, md5sum, archive_name: str = None):
    if uri_validator(url):
        ...
        file_name = os.path.basename(parsed_url.path) if archive_name is None else archive_name
    else:
        file_name = url.split(os.sep)[-1] if archive_name is None else archive_name
