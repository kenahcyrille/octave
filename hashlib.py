...
def download_and_verify(url, pkg_download_dir, md5sum, archive_name: str = None):
    ...
    if os.path.isfile(downloaded_file_path):
        existing_file_md5 = hashlib.md5(open(downloaded_file_path, 'rb').read()).hexdigest()
    ...
    # Verify the MD5
    file_md5 = hashlib.md5(open(downloaded_file_path, 'rb').read()).hexdigest()
    if file_md5 != md5sum:
        os.remove(downloaded_file_path)
...
def build_projects(base_dir):
    ...
    for package, version in build_order:
        print(package_to_file_dict[(package, version)])
